...
@section('content')
    ...
    <form action="{{ route('room-rates.store') }}" method="POST" data-ajax="true">
        @csrf
                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control @error('name') is-invalid @enderror"
                                   id="name" name="name" value="{{ old('name') }}" placeholder="e.g., Weekend Promo, Lebaran Special" required>
                            @error('name')
                                ...
                                <select class="form-select @error('guest_category_id') is-invalid @enderror" name="guest_category_id" data-choices>
                                    <option value="">All Categories</option>
                                     @foreach($guestCategories as $category)
                                        <option value="{{ $category->id }}" {{ old('guest_category_id') == $category->id ? 'selected' : '' }}>{{ $category->name }}</option>
                                    @endforeach
                                </select>
                        ...
                        <div class="mb-3">
                            ...
                            <div class="input-group">
                                ...
                                <input type="number" class="form-control @error('price') is-invalid @enderror"
                                       id="price" name="price" value="{{ old('price') }}" placeholder="Enter price" step="1000" min="0" required>
                                @error('price')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                    ...
                    <div class="card-footer">
                        <div class="d-flex justify-content-end gap-2">
                            ...
                            <button type="submit" data-submit-protect="true" class="btn btn-primary">Save Rate</button>
                        </div>
                    </div>
                </div>
            </div>
