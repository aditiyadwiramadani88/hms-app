...
@section('content')
    ...
    <form action="{{ route('vouchers.update', $voucher->id) }}" method="POST" data-ajax="true">
        @csrf
        @method('PUT')
            ...
            <div class="col-lg-8">
                <div class="card">
                    ...
                    <div class="card-body">
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <input type="number" class="form-control @error('value') is-invalid @enderror" 
                                       id="value" name="value" value="{{ old('value', $voucher->value) }}" step="0.01" required>
                                @error('value')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                        ...
                        <div class="row">
                            <div class="col-md-6 mb-3">
                                ...
                                <input type="number" class="form-control" id="min_booking_amount" name="min_booking_amount" value="{{ old('min_booking_amount', $voucher->min_booking_amount) }}">
                            </div>
                            <div class="col-md-6 mb-3">
                                <label for="max_discount" class="form-label">Max Discount (for % type)</label>
                                <input type="number" class="form-control" id="max_discount" name="max_discount" value="{{ old('max_discount', $voucher->max_discount) }}">
                            </div>
                        </div>
                        ...
                        <div class="row">
                            <div class="col-md-4 mb-3">
                                ...
                                <input type="number" class="form-control" id="usage_limit" name="usage_limit" value="{{ old('usage_limit', $voucher->usage_limit) }}" placeholder="∞">
                            </div>
                            <div class="col-md-4 mb-3">
                                <label for="valid_from" class="form-label">Valid From</label>
                                <input type="text" class="form-control flatpickr-input" id="valid_from" name="valid_from" value="{{ old('valid_from', $voucher->valid_from ? $voucher->valid_from->format('Y-m-d') : '') }}">
                            </div>
                            <div class="col-md-4 mb-3">
                                <label for="valid_until" class="form-label">Valid Until <span class="text-danger">*</span></label>
                                <input type="text" class="form-control flatpickr-input" id="valid_until" name="valid_until" value="{{ old('valid_until', $voucher->valid_until->format('Y-m-d')) }}" required>
                            </div>
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <div class="row">
                                @php $selectedTypes = $voucher->applicable_room_types ?? []; @endphp
                                @foreach($roomTypes as $type)
                                    <div class="col-md-4">
                                        <div class="form-check">
                                            <input class="form-check-input" type="checkbox" name="applicable_room_types[]" value="{{ $type->id }}" id="rt_{{ $type->id }}" {{ in_array($type->id, $selectedTypes) ? 'checked' : '' }}>
                    ...
                    <div class="card-footer text-end">
                        ...
                        <button type="submit" data-submit-protect="true" class="btn btn-primary">Update Voucher</button>
                    </div>
                </div>
            </div>
        </div>
